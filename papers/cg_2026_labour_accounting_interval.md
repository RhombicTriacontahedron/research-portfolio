...
The charge is imposed on a rest point of stated laws. No class chooses anything here, so it is not an equilibrium object, and constant shares are the class in which the rest point is written. Nothing below shows that restriction to be without loss against state-contingent policies. Nothing below is a theory of surplus value or a national net social wage.
...
Proposition 3 reparameterises rest points of the laws by the scale of continuation, and it is used below to turn a stated tuple into an induced one. A growth factor is not a free parameter, so exhibiting a configuration that satisfies an inequality is worth nothing until some admissible primitive puts a rest point there.
...
Two things follow that no valuation object delivers. First, the rate exists whether or not anyone wants it: dating the magnitudes always solves, so the question is never whether an inter-temporal ratio can be had, only whether it can be **one**.
