...
*With the technology extended continuously to $\rho=0$ by its Cobb–Douglas limit, where $\psi\to\theta x$ and $s_K\to\theta$, an affine-in-logs value function therefore solves the Bellman equation if and only if $\rho(1-\rho)=0$, that is if and only if $\sigma=1$ or $\sigma=\infty$.*
...
*which is strictly negative for $\sigma\in(1,\infty)$ and zero at $\sigma=1$. For $\sigma<1$ the same expression is strictly positive, but it is then the sign of the formula (5.2) and not of a preferred rate: Proposition 3 does not license (5.2) as one there.*

The sign is carried as a theorem and not as a number: it is a product of a factor whose negativity is read off the closed form and a factor whose sign is exactly the sign of $\rho$. Both controls fire on the formula (5.2), whose sign Proposition 5 reads for every $a_1<1$: at $\sigma=\tfrac12$ the derivative reverses, and at $\sigma=1$ it is identically zero — as it must be, since Cobb–Douglas shares do not move with $B$ at all. To be sure, at $\sigma=\tfrac12$ and the Section 6 cell the tangent sits at $a_1=-0.4$, outside the domain on which Proposition 3 licenses its coefficients as a value function, so the reversal that carries weight there is the exact model’s, reported in Section 6 (0.2223, 0.3531, 0.4777), which needs no approximation.
...
Under the approximation each class’s payoff in the tax is again $M\ln(1-\tau)+N\ln\tau$ plus terms free of $\tau$ — the transitions are log-linear by construction, so the argument of Section 2(ii) goes through verbatim. On $\rho\in(0,1)$ the transition matrix is entrywise non-negative and felicity is non-decreasing in the states, so the approximating Bellman operator preserves monotonicity and gives $a,c\ge0$; hence $M=(1-\omega)+\beta a \ge 1-\omega>0$ for the owner, and $N=\beta c>0$ because Proposition 3 puts $\tau_C = N/(M+N)$ in $(0,1)$ there. The second derivative is $-M/(1-\tau)^{2}-N/\tau^{2}<0$, so the stationary point is a maximum and not merely a critical point. With (6.1) the Pareto set in the tax is therefore $[\tau_C,\tau_W]$, a non-degenerate interval at every parameter with $\rho\in(0,1)$, and $(0,\tau_C)$ is the set of rates both classes strictly prefer to raise. Neither bound is available for $\rho<0$, where the monotonicity route is closed and Proposition 3’s interiority fails.
...
Across the five values of $B$ computed, capital’s share at the realised stationary ratio rises with $B$, as (i) of Section 2 requires, and the exact $\tau_C$ on constant policies falls monotonically, as Proposition 5 predicts. The controls fire on the exact model too: at $\sigma=\tfrac12$ the exact $\tau_C$ *rises* with $B$ (0.2223, 0.3531, 0.4777 at $\ln B=-1,0,1$), and at $\sigma=1$ it does not move at all (0.4012 at every $B$, which is Section 2(ii) at $\alpha=\theta$). The frozen-share column moves in the right direction and is wrong by up to 0.299 across the five rows. The tangent (5.2) is a different matter, and the point at which it is evaluated decides the figure. At the printed column — rates optimised from $x_0=0$, the stationary ratio read off the path they induce — the tangent misses the exact $\tau_C$ by up to $1.6\times10^{-3}$ across the five rows. At the self-consistent stationary ratio, where the rates are re-optimised from the stationary state itself, the two coincide exactly. Started there, the exact path never leaves $x^{*}$, and neither does the tangent model’s, which agrees with (6.2) in level and slope at $x^{*}$. The gradient of a payoff with respect to constant policies depends only on the path and on the slopes of the transitions and of felicity along it, and here those slopes are the coefficients of (5.1) at $\bar x=x^{*}$. The exact first-order conditions are therefore the tangent model’s, and (5.2) is an exact critical point of the constant-policy problem. That is a statement about one starting state, not a solution of the exact economy: from $x_0=0$ the gap is the $1.6\times10^{-3}$ above. At $\ln B\in\{-1,0,1\}$ the grid finds no other maximum, and the computed gap is zero to machine precision. The second point is the one the approximation is built at; the first is the one the table prints.
