...
The vanishing-tax path is not a destination capital reaches by choosing technique. Lemmas 3–5 and Propositions 3 and 5 are a contradiction: if the tax goes to zero, the public stock and then labour go to minus infinity, and capital income follows them down, which cannot sit with bounded capital income. That rules out a vanishing tax with capital income bounded away from zero. What rules one out along the owner’s *own* optimal path is the floor of Proposition 10.

What the assigned owner will not shut is the public stock that reproduces the labour he hires. Next period’s public stock enters his continuation with a strictly positive loading on the domain $\theta_R<1$, $\gamma>0$. The upkeep of labour he does not own is worth something to him.[^1] Machines that survive ($\delta<1$) split the map from after-tax profit into new capital; they do not remove that loading. In their optimal programmes Chamley and Judd send a capital-income tax to zero when the revenue is a transfer, and Judd (1999) keeps the zero when the spending is chosen. What this paper changes is what the revenue buys, which instrument pays for it and who sets the rate: an input into the labour the owner hires, funded by the capital tax alone, at the owner’s choice. At $\delta=1$ the floor needs the second of these: given a wage tax as well, the owner still funds the stock but sets the capital tax to zero (Proposition 15).
...
Two established routes to a nonzero long-run capital tax are not this paper’s. Saez (2013) overturns the zero by dropping linearity: with a two-bracket progressive schedule and a time-varying exemption threshold, progressive capital taxation redistributes initial wealth far more efficiently than the linear taxation the literature after Chamley and Judd has studied. Jacobs and Schindler (2012) overturn it with risk: among ex ante identical households facing non-insurable labour-income shocks the optimal capital tax is always nonzero, and positive under empirically grounded conditions. The tax here is linear, there is no risk, and the two classes are not ex ante identical. What delivers the floor here is what the revenue buys and which instrument may pay for it.
...
The map $k\mapsto\ln\bigl((1-\delta)e^{k}+i(1-\tau)e^{E_C}\bigr)$ is not affine in $k$ when $\delta<1$. Affine-in-logs closure of the owner’s Bellman equation therefore fails even at $\sigma=1$. At $\delta=1$ and $\sigma=1$ that closure is exact: the owner’s value is affine in logs, and he sets the same tax $\tau_C$ at every state (Proposition 2). The closed forms that make the one-generation peaks constants are not available for $\delta<1$.

Where $V$ is differentiable, the owner’s first-order condition in $\tau$ at an interior best response, holding $(d,i)$ fixed, equates the marginal loss on the after-tax pie to the marginal value of $G'$ through future $L$ (Proposition 1). On the domain $\theta_R<1$, $\gamma>0$ that second term is positive, and at $\sigma=1$ it is at least $\beta^2(1-\omega)(1-\alpha)\gamma(1-\theta_R)$ (Proposition 7). At $\sigma=1$ the candidate is interior at every finite state whether or not $V$ is differentiable (Proposition 6).
...
The sections follow that argument in order. *Environment* sets out the two classes, the CES technology and the three laws of motion, and says who chooses what. *Laws that survive* isolates the two facts that do not depend on the rate of wear. *Stationary assigned tax* writes the owner’s exact first-order condition, shows that at $\delta=1$ and $\sigma=1$ it gives the one-generation peak at every state, and states what a stationary peak would need when capital lasts. *Paths, not only stationary points* turns to the Chamley–Judd object itself, a tax path, and shows, using no optimality condition, that on a compact share set no feasible path combines a vanishing tax with bounded composites, and that at Cobb–Douglas a tax held below a positive threshold empties the economy. *Interior policy at a given state* proves that a best response exists at every state. *A floor that does not move with the state* proves the floor $\bar\tau$ and the sharper floor at a tax that does not raise the capital stock. *Stationary first-order condition* records which substitutions would make the peak a closed form in $\delta$, and why none of them is licensed. *Contact with Chamley and Judd* states what the contact rests on, and what a second tax instrument does to it. The Conclusion collects the answer.
